...
//
// Log: /tmp/brain19_v11.log
// Goal: decoder loss < 2.0 (V10 baseline at epoch 80)
//
...
int main() {
    g_log.open("/tmp/brain19_v11.log");
    log("=== Brain19 V11 Training Run ===");
    log("");
    ...
    // ── Step 6: Language Training (V9 settings + V11 data) ──────────────────
    log("");
    log("[6/6] Language Training V11...");
    log("  Settings: unified concepts, cosine LR decay, 150 epochs");
    log("  V10 baseline: best loss 2.177 @ epoch 80");
    log("  V11 changes: h 74D→90D (64D token + 16D FlexDetail + 10D DimCtx)");
    log("  V11 block-aware LR: token×1.0, flex×0.3, dimctx×0.1");
    log("  V11 advantage: " + std::to_string(ltm.total_relation_count() - initial_relations)
        + " inherited properties + 16D flex detail injection + graph-trained embeddings");
    log("");
    ...
    log("Log saved to: /tmp/brain19_v11.log");
