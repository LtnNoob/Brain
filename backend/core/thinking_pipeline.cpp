...
{
    // Delegate to execute_with_goal with default exploration goal
    GoalState default_goal = GoalState::exploration_goal({}, "");
    return execute_with_goal(seed_concepts, std::move(default_goal), context,
                             ltm, stm, brain, cognitive, curiosity,
                             registry, embeddings, understanding,
                             kan_validator, gdo, refinement_loop);
}
