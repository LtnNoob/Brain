...
DecoderOutput KANDecoder::decode(
    ...
    for (size_t t = 0; t < max_tokens; ++t) {
        ...
        // 3-block hidden state evolution (matching training loop)
        ...
        // Block 2: Dims 64..64+flex_dim-1 (Flex): h = 0.9*h + 0.1*flex_detail
        if (flex_dim_ > 0) {
            auto tok_concept = tokenizer.token_to_concept(token);
            if (tok_concept) {
                auto flex_emb = concept_embeddings.get_or_default(*tok_concept);
        ...
        // Block 3: Dims 64+flex_dim..H-1 (DimCtx): slow decay
