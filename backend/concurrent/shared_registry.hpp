...
public:
    ...
    bool create_model(ConceptId cid) {
        ...
        if (ok) {
            model_mutexes_[cid]; // pre-create per-model mutex
        }
        return ok;
    }
    ...
    size_t ensure_models_for(const LongTermMemory& ltm) {
        ...
        for (auto id : reg_.get_model_ids()) {
            model_mutexes_[id]; // no-op if exists
        }
        return created;
    }
    ...
    // RAII guard for per-model locking
    class ModelGuard {
    public:
        ModelGuard(SharedRegistry& reg, ConceptId cid)
            : reg_(reg), cid_(cid), model_(reg.lock_model_for_training(cid)) {}
        ~ModelGuard() { if (model_) reg_.unlock_model(cid_); }
...
private:
    ...
    mutable std::unordered_map<ConceptId, std::mutex> model_mutexes_;
};
